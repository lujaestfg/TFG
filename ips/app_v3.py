...
<input type="number" id="ruleInput" placeholder="Rule ID" />
  ...
  <h2>Listado de Pods con etiquetas que contienen 'seguridad'</h2>
    <table border="1">
        <tr>
        ...
        <tr>
            ...
            <td>{{ pod.value }}</td>
            <td>
                <form method="POST" action="{{ url_for('editar_etiqueta') }}" style="display:inline-block;">
                    <input type="hidden" name="namespace" value="{{ pod.namespace }}">
                    <input type="hidden" name="name" value="{{ pod.name }}">
                    <input type="hidden" name="key" value="{{ pod.key }}">
                    <select name="nuevo_valor">
                        {% for val in valores %}
                        <option value="{{ val }}" {% if val == pod.value %}selected{% endif %}>{{ val }}</option>
                        {% endfor %}
                    </select>
                    <button type="submit">Actualizar</button>
                </form>
                <form method="POST" action="{{ url_for('eliminar_etiqueta') }}" style="display:inline-block;">
                    <input type="hidden" name="namespace" value="{{ pod.namespace }}">
                    <input type="hidden" name="name" value="{{ pod.name }}">
                    <input type="hidden" name="key" value="{{ pod.key }}">
  ...
  async function fetchRules() {
    ...
    rules.forEach((rule, index) => {
      ...
      editBtn.onclick = () => {
        ...
        document.getElementById('ruleInput').disabled = false;  // ✅ Habilita el campo ID
    ...
    for pod in pods:
        ...
        for key, val in etiquetas.items():
            if "seguridad" in key.lower():
                pods_con_seguridad.append({
                    "name": pod.metadata.name,
                    "namespace": pod.metadata.namespace,
                    "key": key,
                    "value": val
                })
                break  # Evita duplicados si hay múltiples claves "seguridad"
...
@app.route("/eliminar_etiqueta", methods=["POST"])
