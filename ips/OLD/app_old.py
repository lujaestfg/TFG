  ...
  <script>
    ...
    async function updatePodDetails() {
      ...
      const payload = {
        alerts: [{
          annotations: {
            summary: JSON.stringify({ alert: { signature_id: alertRuleId }, src_ip: data.ip })
          }
        }]
      };
...
def alert():
    ...
    try:
        ...
        with lock:
            # Only check PREVENTION_IDS if sig_id is an int, else skip blocking
            if (isinstance(sig_id, int) and sig_id not in PREVENTION_IDS):
